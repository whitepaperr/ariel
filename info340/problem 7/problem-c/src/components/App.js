...
function App(props) {
  ...
  const [runnerSelect, setRunnerSelect] = useState(false);

  let displayedData = props.gameData.filter((team) => {
    if(teamName === '') {
      return true;
    } else if(teamName === team.winner) {
      return true;
    } else if(runnerSelect === true && teamName === team.runner_up) {
      return true;
    }
    return false;
  })

    function applyFilter(teamFilter, runnerBoolean){
      setTeamName(teamFilter);
      setRunnerSelect(runnerBoolean);  
    }
