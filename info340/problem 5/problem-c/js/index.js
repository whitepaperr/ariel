...
let lostTo = lostgame.map(extractOpponent);
...
lostgame.forEach(function(game) {
...
function mostYardsPassing(game1, game2) {
  if (game1.passing_yards === undefined) {
    return game2;
  } else if (game2.passing_yards === undefined) {
    return game1;
...
let avgScoreDifference = huskyGames2016.reduce(function(acc, game) {
  let diff = game.home_score - game.opponent_score;
  let total = acc + diff;
  return total/huskyGames2016.length;
}, 0);
