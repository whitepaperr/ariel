...
attendess_distriution <- boxplot(num_attendees)
...
num_in_wa <- sum(str_detect(locations, pattern = "WA"))
...
# Note, you should count the number of locations that *mfitatch* the parameter
# put into the function, so `Seattle` should be a match for "Seattle, WA"
count_in_location <- function(name){
  num_in_name <- sum((str_detect(locations, name)))
  sprintf("There were %s protests in %s", num_in_name, name)
}
...
# `stringr` package
states <- str_sub(locations, nchar(locations) -2, nchar(locations))
...
count_on_date <- function(date){
  num_in_date <- sum((str_detect(dates, date)))
  sprintf("There were %s protests on %s", num_in_date, date)
}
...
# https://www.nytimes.com/article/george-floyd-protests-timeline.htmls
...
by_month_2019 <- table(months(in_2019))
July_2019 <- by_month_2019[6]
July_2020 <- by_month_table[6]
change_july_protests <- abs(July_2019 - July_2020)
...
# `high_level_table`
high_level_table <- high_level_purpose
...
my_function <- function(nnn){
  ans <- sum((str_detect(purpose, nnn)))
  sprintf("There were %s prtests for %s.", ans, nnn)
}
my_function("Healthcare")
