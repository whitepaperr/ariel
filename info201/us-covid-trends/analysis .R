...
# that has the number of *new* cases each day (hint: look for the `lag`
# function).
national <- national %>%
  mutate(new_cases = c(cases) - lag(cases))
...
national <- national %>%
  mutate(new_deaths = c(deaths) - lag(deaths))
...
state_by_day <- states %>%
  ...
  summarize(state_total = sum(cases, na.rm = TRUE))
...
county_by_day <- counties %>%
  group_by(date, add = FALSE) %>%
  summarize(county_total = sum(cases, na.rm = TRUE))
...
totals_by_day <- merge(state_by_day, county_by_day)
...
sum_county_to_state <- counties %>%
  group_by(date, add = FALSE) %>%
  summarize(state, date, county_total =sum(cases), .groups = "drop") %>%
  arrange(date)
...
state_highest_difference <- has_discrepancy %>%
  filter(abs(cases - county_total) == max(abs(cases - county_total))) %>%
  select(state)
...
# How many days have the same number of cases for county and state?
# `same_total`
same_total <- all_totals %>%
  filter(county_total == state_total, na.rm = TRUE) %>%
  nrow

# Which location has the highest difference between the cases and the deaths?
# `highest_diff`
hightest_diff <- loc_county %>%
  drop_na(cases) %>%
  drop_na(deaths) %>%
  filter(abs(cases - deaths) == max(abs(cases - deaths))) %>%
  select(location)

# For how many days are the number of new cases unavailable in the all_totals dataframe?
# `num_NAs`
num_NAs <- all_totals %>%
